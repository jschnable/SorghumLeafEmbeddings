...
- `data/provided/gwas_covariates_leaf_area_flowering_time.csv`: genotype-level GWAS covariates used by the embedding GWAS. Columns are `mask_pixels_blue` (genotype BLUE of raw OpenCV mask area, regenerated by `scripts/make_gwas_leaf_area_covariate.py`) and `days_to_flower_blue`; the GWAS script z-scores them internally.
...
python scripts/run_gwas_panicle.py \
  ...
  --covariate-cols mask_pixels_blue,days_to_flower_blue \
