...
These are frozen final figure inputs; the original water calculation and export wrappers are archived under `deprecated/scripts/`.
The original exporter produced the complete-covariate water sample and current test results,
...
The committed expression CSV is the final input used by panel e. Its original single-marker test is archived.
...
`figures/supplemental/FigS12_cyp97b_jar1_hotspots/human_score_blue_nebraska.csv` phenotype
(`human_score_blue`) with the archived single-marker implementation, markers
