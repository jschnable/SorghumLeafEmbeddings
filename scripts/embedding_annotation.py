...
The feature extractor calls :func:`annotate_embeddings` to attach the shared
metadata schema to its output NPZ.
...
def annotate_embeddings(
    ...
    df["disease_exg"] = np.log(pd.to_numeric(df["pct"], errors="coerce")/(100 - pd.to_numeric(df["pct"], errors="coerce")))
