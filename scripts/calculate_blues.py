...
def model_plot_means(data: pd.DataFrame, traits: list[str], args: argparse.Namespace) -> tuple[pd.DataFrame, list[str]]:
    ...
    frame["mask_pixels"] = pd.to_numeric(frame["mask_pixels"], errors="coerce")
    frame.loc[~np.isfinite(frame["mask_pixels"]), "mask_pixels"] = np.nan
    leaf_present = bool(frame["mask_pixels"].notna().any())
    ...
    if leaf_present and args.environment == "all":
        ...
        grp = plot_means.groupby("environment")["mask_pixels"]
        within = (plot_means["mask_pixels"] - grp.transform("mean")) / grp.transform("std")
        ...
        plot_means["mask_pixels_scaled"] = zscore_column(plot_means["mask_pixels"])
        fixed_covariates = ["mask_pixels_scaled"]
...
def load_data(args: argparse.Namespace) -> tuple[pd.DataFrame, list[str]]:
    ...
    # Leaf-area covariate is the raw OpenCV mask pixel area carried in the embedding
    # npz (extract_embeddings.py), read straight off the scores table rather than
    # copied from the metadata file. model_plot_means z-scores it.
    if "mask_pixels" not in data.columns:
        data["mask_pixels"] = np.nan
    return data, traits
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Workers for the lme4 per-trait loop (R parallel::mclapply). Each fit is "
        "single-threaded but memory-bandwidth heavy, so throughput peaks around 8 workers "
        "and DEGRADES past that (benchmarked: 8 workers beat 24 by ~30% on a 24-core box). "
        f"Default: min(cores, 8) = {min(os.cpu_count() or 1, 8)} here. Raise it only on "
        "machines with more memory bandwidth; pass 1 for serial.",
    )
    parser.add_argument(
        "--progress-every",
        type=int,
