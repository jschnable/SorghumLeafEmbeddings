...
def read_image_paths(image_input: Path | str, image_col: str, image_root: Path | None = None) -> list[Path]:
        ...
        if path.suffix.lower() == ".csv":
            ...
            for value in df[image_col].dropna().astype(str):
                ...
                if p.is_absolute():
                    ...
                    paths.append(image_root / p)
                else:
                    csv_relative = base / p
                    repo_relative = REPO_ROOT / p
                    if csv_relative.is_file() and repo_relative.is_file() and csv_relative.resolve() != repo_relative.resolve():
                        raise ValueError(f"Ambiguous image path {p}; select its base with --image-root")
                    paths.append(csv_relative if csv_relative.is_file() else repo_relative if repo_relative.is_file() else csv_relative)
        elif path.is_dir():
...
def crop_bgr_to_model_rgb(crop_bgr: np.ndarray) -> np.ndarray:
    """Convert one extracted BGR crop to the shared resized RGB uint8 model input."""
    crop_rgb = cv2.cvtColor(crop_bgr, cv2.COLOR_BGR2RGB)
    return resize_for_model(crop_rgb)


def normalized_chw_tensor_from_rgb(rgb: np.ndarray) -> torch.Tensor:
    """Convert shared RGB uint8 model input to a CHW tensor normalized to [-1, 1]."""
    if rgb.dtype != np.uint8:
        raise TypeError(f"Expected uint8 RGB image, got {rgb.dtype}")
    tensor = torch.from_numpy(rgb).permute(2, 0, 1).to(torch.float32)
    return tensor.div(255.0).sub(0.5).div(0.5)
...
class Sam3Extractor:
    ...
    def metadata(self) -> dict[str, object]:
        return {
            ...
            "backend_preprocessing": f"shared_bgr_to_rgb_cv2_area_resize_{MODEL_INPUT_SIZE}_then_sam3_processor",
        }

    def embedding(self, crop_bgr: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        model_rgb = crop_bgr_to_model_rgb(crop_bgr)
...
class Dino2Extractor:
    ...
    def metadata(self) -> dict[str, object]:
        return {
            ...
            "backend_preprocessing": f"shared_bgr_to_rgb_cv2_area_resize_{MODEL_INPUT_SIZE}_normalize_0.5",
        }

    def embedding(self, crop_bgr: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        model_rgb = crop_bgr_to_model_rgb(crop_bgr)
        x = self.preprocess_model_rgb(model_rgb).unsqueeze(0).to(self.device)
        with torch.inference_mode():
            out = self.model.forward_features(x)
        patch_tokens = out["x_norm_patchtokens"]
        return pool_features(patch_tokens)

    def preprocess(self, rgb: np.ndarray) -> torch.Tensor:
        """Convert an RGB image through the shared resize and DINO2 tensor normalization."""
        return self.preprocess_model_rgb(resize_for_model(rgb))

    def preprocess_model_rgb(self, model_rgb: np.ndarray) -> torch.Tensor:
        """Convert the shared resized RGB uint8 image to DINO2's tensor input."""
        return normalized_chw_tensor_from_rgb(model_rgb)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image-root", type=Path, help="Base directory for relative CSV image paths; otherwise resolve beside the CSV or from the repository root.")
...
def main() -> None:
    ...
    image_paths = read_image_paths(args.image_input, args.image_col, args.image_root)
    if not image_paths:
    ...
    missing_images = [p for p in image_paths if not p.is_file()]
    if missing_images:
        raise SystemExit(f"{len(missing_images)} input images not found; first: {missing_images[0]}")
