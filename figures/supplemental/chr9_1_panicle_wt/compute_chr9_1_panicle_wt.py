#!/usr/bin/env python3
"""Box-panel data + panicle LOCO-MLM (5 PC) p-value for the chr9:1.77 Mb LysM-RLK lead
marker (9:1768703:G:T) vs. single_plant_panicle_dry_weight_g, MI2020 only.

Marker: 9:1,768,703 (G>T), same lead marker as figures/lysm_rlk_story (see its meta.json;
candidate Sobic.009G019100). Phenotype: single_plant_panicle_dry_weight_g, per-genotype
mean of MI2020 plants (source data/externalsourcerequired/sorghum_trait_data_v2.2.zip,
per_location_traits/MI2020.tsv). Writes box_data.csv (genotype, peak_dose,
single_plant_panicle_dry_weight_g) and mlm_pvalues.json.
"""
...
OUT = Path("figures/supplemental/chr9_1_panicle_wt")
...
mi = int(np.where((mdf.CHROM.values == "9") & (mdf.POS.values == PEAK))[0][0])
ref, alt = mdf.iloc[mi]["REF"], mdf.iloc[mi]["ALT"]
log(f"marker 9:{PEAK} {ref}>{alt}")
dose = pd.Series(geno.subset_markers(np.array([mi])).to_numpy().ravel(), index=ids)

# ---- LOCO-MLM test (5 PCs), same model as compute_chr4_69_panicle_wt.py ----
...
pv = {"marker": f"Chr09:{PEAK:,} ({ref}>{alt})", "trait": TRAIT, "env": "MI2020",
