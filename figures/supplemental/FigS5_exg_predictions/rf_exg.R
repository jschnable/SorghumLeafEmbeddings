...
setwd(file.path(.repo_root, "figures/supplemental/FigS5_exg_predictions"))
library(cowplot)
library(paletteer)
...
sam3_exg_preds <- read_csv('sam3_embedding_exg_rf_image_predictions.csv.gz')
...
sam3_scatter <- ggplot(sam3_exg_preds, aes(observed, predicted)) + 
  ...
  theme_use
sam3_scatter
dino2_exg_preds <- read_csv('dino2_embedding_exg_rf_image_predictions.csv.gz')
...
ggsave('rf_exg_scatterplot_from_code.png', plot = scatter, dpi = 300, width = 4, height = 8, bg = 'white')
