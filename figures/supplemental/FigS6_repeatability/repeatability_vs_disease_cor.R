...
setwd(file.path(.repo_root, "figures/supplemental/FigS6_repeatability"))
library(tidyverse)
library(paletteer)
library(cowplot)
...
sam3_r <- read_csv('sam3_heritability_Nebraska2025.csv.gz') %>% 
  filter(h2_reliable) %>% 
  select(trait, broad_sense_h2)
dino2_r <- read_csv('dino2_heritability_Nebraska2025.csv.gz') %>% 
  filter(h2_reliable) %>% 
  select(trait, broad_sense_h2)
