...
# Plot environment-specific allele effects and their named p-values.
...
plot_candidate_expression <- function(expr_path, geno_col, marker_tbl, short_label, colors, candidate_id, pval = NULL)
{
  ...
  if(!file.exists(expr_path)) stop('Required candidate expression input not found: ', expr_path)
...
plot_gloss_boxplot <- function(gloss_path, geno_col, marker_tbl, short_label, colors, allele_labels = NULL, pval = NULL)
{
  ...
  # data/generatable/loci/chr2_gdsl/make_chr2_peak_figure.py, which uses 'ref'/'alt' for the same
