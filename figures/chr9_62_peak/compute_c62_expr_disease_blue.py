...
disease_exg/human_score are now genotype BLUEs, not raw per-image means; pct dropped
(redundant with disease_exg, and no longer present in repr_traits_3.csv).
Writes expr_disease_tests_blue.json + expr_disease_table_blue.csv."""
...
# per-genotype disease traits: genotype BLUEs (not raw per-image means)
...
targets = {"disease_exg": disease_exg_blue, "human_score": human_score_blue}
...
json.dump(R, open(OUT / "expr_disease_tests_blue.json", "w"), indent=2)
...
pd.DataFrame(rows).to_csv(OUT / "expr_disease_table_blue.csv", index=False)
...
print(f"{'gene':<18}{'disease_exg':>24}{'human_score':>24}")
for gid in GENELIST:
    if gid not in R: continue
    d = R[gid]; mark = " <-- VQ" if gid == VQ else ""
    star = " *" if any(d[t].get("partial_p", 1) < nb for t in targets) else ""
    print(f"{gid:<18}{fc(d['disease_exg']):>24}{fc(d['human_score']):>24}{star}{mark}")
...
print("DONE — expr_disease_tests_blue.json")
