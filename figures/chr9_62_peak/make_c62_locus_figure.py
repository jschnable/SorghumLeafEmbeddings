#!/usr/bin/env python3
"""chr9:62.2 locus panel. Manhattan of the peak embeddings + LD-to-lead + gene models.Disease-severity peak; driving block A (~62.27-62.35 Mb) contains the lead 62,301,540 (inside
Sobic.009G249800). Highlighted: JAR1-like jasmonate-amino-acid ligase Sobic.009G249900 (top
cis-eQTL, best defense candidate) and the lead gene Sobic.009G249800. Best coding tag
Sobic.009G249700 Ile446Arg r2=0.92.
Regenerate: python figures/chr9_62_peak/make_c62_locus_figure.py"""
...
C_JAR = "#2c7fb8"; C_LEAD = "#7a4fa3"
MISS = 62_293_721   # Sobic.009G249700 Ile446Arg, best coding tag r2=0.92
CAND = {"Sobic.009G249900": ("Sobic.009G249900\nJAR1 JA-Ile ligase (top eQTL)", 62.360, C_JAR),
        "Sobic.009G249800": ("Sobic.009G249800\nlead gene", 62.255, C_LEAD)}
...
ymax = -np.log10(reg.p_value.min()); ax_man.set_yticks([y for y in range(0, 16, 2) if y <= ymax + 1])
...
ax_gene.annotate("Ile446Arg\n(r²=0.92)", xy=(MISS / 1e6, FWD[1]), xytext=(MISS / 1e6, 1.02), ha="center", va="bottom",
...
fig.savefig(D / "c62_locus.png", dpi=300, bbox_inches="tight")
print("wrote", D / "c62_locus.png")
