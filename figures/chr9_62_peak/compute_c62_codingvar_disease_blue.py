...
Same test as compute_c62_codingvar_disease.py, but disease_exg/human_score are now genotype BLUEs
(data/generatable/blues/{nebraska_exg_logit,allsites_human_scores}/blues_Nebraska2025.csv) instead
of a raw per-image genotype mean. Kept as inline PANICLE_MLM_LOCO_MULTI (not
scripts/run_single_marker_test.py) because the conditional tests need an extra marker-dosage
covariate that the shared single-marker-test CLI doesn't support.
Writes codingvar_disease_blue.json."""
...
disease_exg_blue = pd.read_csv("data/generatable/blues/nebraska_exg_logit/blues_Nebraska2025.csv").set_index("genotype")["ExG_P20_disease_pct"]
human_score_blue = pd.read_csv("data/generatable/blues/allsites_human_scores/blues_Nebraska2025.csv").set_index("genotype")["human_score"]
TR = {"disease_exg": disease_exg_blue, "human_score": human_score_blue}
...
json.dump(R, open(OUT / "codingvar_disease_blue.json", "w"), indent=2)

def fmt(d): return f"n={d['n']} minor={d['n_minor']} b*={d['beta_std']:+.3f} p={d['p']:.2e}"
for tr in TR:
    print(f"\n===== {tr}: MARGINAL (LOCO-MLM + 5PC; BLUE) =====")
...
log("DONE — codingvar_disease_blue.json")
