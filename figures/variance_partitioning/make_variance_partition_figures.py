...
figure CSV plus a stacked-bar ``.png`` of the variance composition per
...
def plot_stacked(grouped: pd.DataFrame, out_prefix: Path, blocks: list[dict] | None = None) -> None:
    ...
    # Cap at 6.5 in so 300 dpi exports stay within journal max width (1950 px).
    W = min(6.5, max(4.0, 0.6 * n))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-prefix", required=True, type=Path, help="Output path prefix; writes .csv and .png")
    parser.add_argument(
        "--h2-rows",
...
def main() -> None:
    ...
    print(f"Wrote {args.out_prefix}.csv/.png ({grouped['trait'].nunique()} traits){'; ' + summary if summary else ''}")
