...
Same tests as compute_chr4_tests.py, with three fixes:
1. human_score/disease_exg (T2/T3) are now genotype BLUEs (data/generatable/blues/
   {allsites_human_scores,nebraska_exg_logit}/blues_Nebraska2025.csv), not a raw per-image mean.
2. Leaf TPM moved from figures/embedding_gwas_hotspots/ExpressionData/ (stale, gone) to
   data/externalsourcerequired/tpm/ (current canonical location, same data).
3. T3's genotype-PC covariate table used to be read from a scratch directory belonging to a
   different machine/session (`/tmp/claude-1000/-home-james-leaf-imaging-.../genopca.eigenvec`)
   that no longer exists here. The same genome-wide 5-PC table is already checked into every
   sibling peak directory as `geno_pcs.eigenvec` (identical file, verified byte-for-byte across
   directories) -- reused from figures/chr4_lutein_peak/geno_pcs.eigenvec instead of regenerating.

Writes chr4_tests_results_blue.json and prints a summary.
"""
...
def log(m): print(f"[tests-blue] {m}", flush=True)
...
# ---- phenotypes: genotype BLUEs, not raw per-image means ----
...
pcs = pd.read_csv("figures/chr4_lutein_peak/geno_pcs.eigenvec", sep="\t").rename(columns={"#IID": "genotype"}).set_index("genotype")
...
json.dump(R, open(OUT / "chr4_tests_results_blue.json", "w"), indent=2)
log("DONE — wrote chr4_tests_results_blue.json")
