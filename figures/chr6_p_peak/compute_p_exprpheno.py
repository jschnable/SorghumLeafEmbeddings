#!/usr/bin/env python3
"""Expr -> phenotype test for the chr4:4.7 peak (part B, mirrors compute_chr4b_allgenes.py).
...
OUT = Path("figures/chr6_p_peak")
VCF = "data/externalsourcerequired/vcf/sorghum_925genotypes_filtered_v3.vcf.gz"
LEAD = 58_476_610
# DIMS = 3 lowest-p peak dims at the lead (embedding_mean_30 8e-15, std_873 1.3e-11, mean_186 1.2e-10)
PEAK_DIMS = ["embedding_mean_30", "embedding_std_873", "embedding_mean_186"]
...
gt = subprocess.run(["bcftools", "query", "-r", f"6:{LEAD}-{LEAD}", "-f", "[%GT\t]\n", VCF], capture_output=True, text=True).stdout.strip().split("\t")
