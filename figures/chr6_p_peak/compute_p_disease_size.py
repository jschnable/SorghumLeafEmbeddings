#!/usr/bin/env python3
"""chr6:58.5 (P pigment) — is the peak marker (58,476,610) associated with DISEASE, COLOR, or SIZE?
...
mi = int(np.where((mdf.CHROM.values == "6") & (mdf.POS.values == LEAD))[0][0])
...
print(f"\n===== lead 58,476,610 (ALT=A minor) -> image traits (LOCO-MLM+5PC); Bonferroni {0.05/ntests:.1e} =====")
for grp, cols in GROUPS.items():
    print(f"  [{grp}]")
    for c in cols:
