#!/usr/bin/env python3
"""chr6:58.5 P-locus panel. Manhattan of the peak embeddings + LD-to-lead + gene models.Pigment locus (3-deoxyanthocyanidin purple/tan response). Highlighted: the literature-cloned
P gene Sobic.006G226800 (= Sb06g029550, flavanone 4-reductase; causal Cys252Tyr is absent from
our 925-genotype VCF) and the neighbouring ANR Sobic.006G227300 (single strongest cis-eQTL,
p=5e-35). Both sit in the tandem flavonoid/proanthocyanidin cluster distal to the lead.
Regenerate: python figures/chr6_p_peak/make_p_locus_figure.py"""
...
C_P = "#2c7fb8"; C_ANR = "#7a4fa3"
CAND = {"Sobic.006G226800": ("Sobic.006G226800\nP / flavanone 4-reductase", 58.560, C_P),
        "Sobic.006G227300": ("Sobic.006G227300\nANR (strongest eQTL)", 58.612, C_ANR)}
...
ymax = -np.log10(reg.p_value.min()); ax_man.set_yticks([y for y in range(0, 20, 2) if y <= ymax + 1])
...
for _, gn in genes.iterrows():
    ...
    ax_gene.plot([gn.start / 1e6, gn.end / 1e6], [cy, cy], color=c, lw=1.0 if isc else 0.7, zorder=3 if isc else 1, solid_capstyle="butt")
...
ax_gene.annotate("flavonoid / proanthocyanidin cluster", xy=(58.60, 0.06), ha="center", va="bottom", fontsize=5.6, color="#8a6d1a")
...
ax_gene.set_xlabel("Chr06 position (Mb)"); ax_gene.spines[["top", "right", "left"]].set_visible(False)

fig.savefig(D / "p_locus.png", dpi=300, bbox_inches="tight")
print("wrote", D / "p_locus.png")
