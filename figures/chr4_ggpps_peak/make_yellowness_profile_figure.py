#!/usr/bin/env python3
"""Yellowness (b*) profile across the leaf width (bottom margin -> midrib -> top margin),
minor (G/G) vs major (A/A) allele at chr4:65,447,981. Top: mean b* per bin per group (±SE).
Bottom: minor-major difference per bin (±SE). Input: scratch yellowness_profiles.npz."""
...
df = load_yellowness_profiles(SCR)
...
for m, s, c, lab, n in [(mmin, smin, C_MIN, f"minor  G/G  (n={nmin} leaves)", nmin),
                        (mmaj, smaj, C_MAJ, f"major  A/A  (n={nmaj} leaves)", nmaj)]:
...
ax1.set_title("Yellowness across the leaf (bottom margin → midrib → top margin), by chr4:65,447,981 allele", fontsize=9.5)
...
ax2.set_xlabel("position across leaf width   (bin 0 = bottom margin  →  bin 99 = top margin)")
