...
def test_blue_calculation_uses_plot_level_means() -> None:
    ...
    args = Namespace(environment="Nebraska2025", include_leaf_area=False, winsor_strength=0.0,
                     spatial_cols="row,column,block", no_plot_averaging=False)
...
def test_shared_crop_preprocessing_converts_bgr_to_resized_rgb() -> None:
    ...
    model_rgb = extract_embeddings.crop_bgr_to_model_rgb(crop_bgr)
...
def test_dino_preprocess_uses_shared_model_resize() -> None:
    ...
    image[:, extract_embeddings.DEFAULT_CROP_SIZE // 2 :] = 128
    ...
    assert float(tensor[:, :, 558:].mean()) == pytest.approx(128 / 255 * 2 - 1, abs=1e-3)


def test_dino_model_rgb_preprocess_rejects_non_uint8() -> None:
    extractor = object.__new__(extract_embeddings.Dino2Extractor)
...
def test_image_csv_accepts_repository_and_csv_relative_paths(tmp_path, monkeypatch):
    root = tmp_path / 'repo'
    base = root / 'data' / 'provided'
    base.mkdir(parents=True)
    image = root / 'data' / 'photos' / 'leaf.jpg'
    image.parent.mkdir();image.write_bytes(b'image')
    example = base / 'images' / 'example.jpg'
    example.parent.mkdir();example.write_bytes(b'image')
    image_list = base / 'images.csv'
    pd.DataFrame({'image_path':['data/photos/leaf.jpg','images/example.jpg']}).to_csv(image_list,index=False)
    monkeypatch.setattr(extract_embeddings,'REPO_ROOT',root)
    assert set(extract_embeddings.read_image_paths(image_list,'image_path')) == {image,example}


def test_ambiguous_image_csv_requires_explicit_base(tmp_path, monkeypatch):
    root = tmp_path / 'repo';base = root / 'provided';base.mkdir(parents=True)
    for parent in [root,base]:
        (parent / 'leaf.jpg').write_bytes(b'image')
    image_list=base / 'images.csv'
    pd.DataFrame({'image_path':['leaf.jpg']}).to_csv(image_list,index=False)
    monkeypatch.setattr(extract_embeddings,'REPO_ROOT',root)
    with pytest.raises(ValueError,match='Ambiguous image path'):
        extract_embeddings.read_image_paths(image_list,'image_path')
    assert extract_embeddings.read_image_paths(image_list,'image_path',root)==[root/'leaf.jpg']
